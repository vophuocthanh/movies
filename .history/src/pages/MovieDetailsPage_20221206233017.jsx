    ...
    <div className="py-10">
      <div className="w-full h-[600px] relative">
        ...
        <div
          ...
          style={{
            backgroundImage: tmdbAPI.imageBackground(backdrop_path),
          }}
        ></div>
      </div>
...
function MovieCredits() {
  const { movieId } = useParams();
  const { data } = useSWR(tmdbAPI.getCredits(movieId), fetcher);
  if (!data) return null;
  const { cast } = data;
...
function MovieVideos() {
  const { movieId } = useParams();
  const { data } = useSWR(tmdbAPI.getVideo(movieId), fetcher);
  if (!data) return null;
